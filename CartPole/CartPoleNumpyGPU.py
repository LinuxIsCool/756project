...
CONFIG = [
    dict(game="CartPole-v0",
         n_feature=4, n_action=2, continuous_a=[False], ep_max_step=1000, eval_threshold=995),
...
def train(net_shapes, net_params, optimizer, utility, pool):
    ...
    noise_seed = np.concatenate([noise_seed,noise_seed]).reshape([2,N_KID]).T.reshape([N_KID*2])

    # serially train with GPU
    rewards = []
    for k_id in range(N_KID*2):
        reward = get_reward(net_shapes, net_params, env, CONFIG['ep_max_step'], CONFIG['continuous_a'],[noise_seed[k_id], k_id])
